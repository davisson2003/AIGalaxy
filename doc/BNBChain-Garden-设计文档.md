...
| 事件 Feed | 右侧实时滚动的链上事件流（每条网络最多 500 条） | registryWatcher |
| Agent 卡片 | hover 显示 Agent 详情（名称 + 链上地址链接到 bsctrace.com、声誉、ERC-8004 认证） | 链上 storage |
| StatsPanel | 链接状态（Connecting/Live/Mock）、Agent 数、领地分布 | Zustand store |
| 动画系统 | 粒子射线、广播波纹、圆点漂移 | Phaser 3.90 |
| Agent 自动发现 | 监听 ERC-8004 Identity + Reputation Registries，BNBAgent SDK 注册的 Agent 自动出现在地图 | ERC8004Watcher + ReputationWatcher |
| Live Feed 改进 | 显示 Agent 名称 + 链接地址、CLR 清除按钮、自动换行标签 | FeedPanel |
| Top Agents 面板 | 除声誉得分外，额外显示交互次数和最后活跃时间 | 侧边栏组件 |
| 链上注册（扩展） | Agent 通过 BNBGardenRegistry 注册，获得 Action/Migrate/Broadcast 动画能力 | BNBGardenRegistry |
...
|---------|---------|---------|---------|
| ERC-8004 Agent 注册 | 新圆点在领地生成（渐入动画） | 🤖 | 按 initRep 初始化 |
| 声誉更新（ERC-8004） | 圆点大小按声誉重新计算 | ⭐ | 链上声誉同步 |
...
│         │            └───────────────────────────────────┘  │
│  ┌──────▼───────────────────────────────────┐               │
│  │     Zustand Store（persist 持久化）       │               │
│  │  agents[] / feedEvents[]（500/网络）     │               │
│  │  chainStatus / networkCache（网络隔离）   │               │
│  └──────┬───────────────────────────────────┘               │
│         │                                                    │
│  ┌──────▼────────────────────────┐                          │
│  │     数据层（Hooks）             │                          │
...
│  ERC-8004 Identity Registry（主网：0x8004A169...）           │
│  ERC-8004 Reputation Registry（主网：0x8004BAa1...）         │
...
│   ├── chainWatcher.ts       # 轮询 DEX/Lending 协议事件 + ERC8004Watcher（自动发现）
...
interface GardenStore {
  ...
  feedEvents:    FeedEvent[]         // 右侧事件流（最多 500 条/网络）
  chainStatus:   ProviderStatus      // "connecting" | "connected" | "error"
  rpcEndpoint:   string | null       // 当前使用的 RPC URL
  currentNetwork: "mainnet" | "testnet" // 当前网络
  networkCache:  Map<string, Agent[]> // 按网络隔离的状态缓存（localStorage 持久化）
...
> 注：Store 使用 `persist` 中间件同步状态到 localStorage。按网络隔离的缓存防止切换网络时状态泄露。
...
| 4 | `https://bsc.nodereal.io/`（使用 `VITE_NODEREAL_KEY`） | NodeReal | 备选 3 |
| 5 | `https://endpoints.omniatech.io/v1/bsc/...` | Ankr | 最后备选 |

**NodeReal 配置：** API 密钥现在从 `VITE_NODEREAL_KEY` 环境变量加载，而不是硬编码。本地开发在 `.env.local` 中配置：
```
VITE_NODEREAL_KEY=your_nodereal_api_key
```
Vercel 部署时，在 Vercel 控制台中添加 `VITE_NODEREAL_KEY` 到项目环境变量。

`rpc.ts` 在初始化时并行尝试所有端点，选取响应最快且区块高度最新的节点，之后定期检测故障切换。

### 6.2 ERC-8004 Watchers — 自动发现（分离式架构）

Garden 现在运行 TWO 个独立的 Watcher，提高了模块化程度：

**1. ERC8004Watcher — Identity Registry**（🤖 已加入）
- 监听 Identity Registry（主网：`0x8004A169FB4a3325136EB29fA0ceB6D2e539a432` | 测试网：`0x8004A818BFB912233c491871b3d84c89A494BD9e`）
- 监听 `Transfer(from=0x0)` 事件 = 新 Agent 注册
- 调用 `tokenURI(tokenId)` 读取 data:application/json;base64,... 元数据
- 解析 `garden.territory` 并在 Feed 中显示"🤖 agent_name joined"

**2. ReputationWatcher — Reputation Registry**（⭐ 声誉已更新）
- 监听 Reputation Registry（主网：`0x8004BAa17C55a88189AE136b182e5fdA19dE9b63` | 测试网：`0x8004B663056A597Dffe9eCcC1965A193B7388713`）
- 监听声誉更新事件
- 将链上声誉分数同步回 Agent 显示
- Feed 中显示"⭐ agent_name reputation updated: +10"

**数据流：**
```
BNBAgent SDK: identityRegistry.register(agentURI)
       ↓
Identity Registry 发出 Transfer(0x0 → owner, tokenId)
       ↓  getLogs 轮询（≤15 秒）
ERC8004Watcher.tokenURI(tokenId) → 解析 JSON → garden.territory
       ↓
store.addAgent() + Feed "🤖 joined"  →  Phaser.js 地图新圆点
```

**声誉同步流：**
```
Reputation Registry 发出 ReputationUpdated(agentId, score)
       ↓
ReputationWatcher → updateAgent(agentId, reputation)
       ↓
Feed "⭐ reputation updated"  →  圆点大小重新计算
```

### 6.3 registryWatcher — BNBGardenRegistry 事件（动画层）
...
| `NodeReal 返回 401 Unauthorized` | 硬编码的 NodeReal 端点缺少 API 密钥认证 | 使用 `VITE_NODEREAL_KEY` 环境变量；在 `.env.local` 和 Vercel 控制台配置 | ✅ 已修复 |
| `tokenURI 静默丢弃 Agent` | 解析 Agent 元数据时未捕获异常 | 在 ERC8004Watcher.tokenURI() 内部添加 try-catch，并有降级日志 | ✅ 已修复 |
| `Non-indexed tokenId` | 事件日志中 tokenId 不在索引 topics 中，仅在 log.data 编码数据中 | 使用降级方案：`log.topics[3] ?? log.data` 提取 tokenId | ✅ 已修复 |
| `网络切换状态泄露` | 切换网络时旧 feedEvents/agents 混入新网络数据 | 实现按网络隔离的缓存（`networkCache` Map）+ localStorage persist 中间件 | ✅ 已修复 |

---

*BNBChain Garden 设计文档 v1.1 | 内部文档 | 更新于 2026 年 3 月*
