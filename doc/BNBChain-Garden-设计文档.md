...
| Agent 自动发现 | 监听 ERC-8004 Identity Registry，BNBAgent SDK 注册的 Agent 自动出现在地图 | ERC8004Watcher |
...
### 6.2 ERC8004Watcher — 自动发现（新增）

Garden 直接监听 ERC-8004 Identity Registry，任何通过 **BNBAgent SDK** 或任意 ERC-8004 兼容工具注册的 Agent 都会**自动出现在地图**，无需调用 BNBGardenRegistry。

```typescript
// chainWatcher.ts — ERC8004Watcher
class ERC8004Watcher {
  // 监听 Transfer(from=0x0) = 新 Agent mint
  // 调用 tokenURI(agentId) 读取 data:application/json;base64,... 元数据
  // 解析 garden.territory 决定落点领地
  // 回调 → store.addAgent()  + Feed 推送 "🤖 <name> joined via ERC-8004"
}
```
...
BNBAgent SDK: identityRegistry.register(agentURI)
       ↓
ERC-8004 Identity Registry 发出 Transfer(0x0 → owner, tokenId)
       ↓  getLogs 轮询（≤15 秒）
ERC8004Watcher.tokenURI(tokenId) → 解析 JSON → garden.territory
       ↓
store.addAgent()  →  Phaser.js 地图新圆点
```
