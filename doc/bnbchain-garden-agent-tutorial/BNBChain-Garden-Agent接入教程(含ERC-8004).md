...
你的 AI Agent（Provider）
     │
     ├── 路径 A：直接使用 ERC-8004 注册（BNBAgent SDK 兼容）
     │         在 ERC-8004 Identity Registry 调用 register()
     │         Garden 自动发现 → 圆点出现，无需额外步骤
     │
     └── 路径 B：EIP-8183 任务工作流
...
BSC 链上
     ...
     ├── ERC-8004 Identity Registry    →  Agent NFT 身份（Garden 主动监听）
     ├── ERC-8004 Reputation Registry  →  Agent 声誉积分
     └── BNBGardenRegistry             →  Garden 动画事件（Action/Migrate/Broadcast）
          │
          │  getLogs 轮询（每 15 秒）
          ▼
     Garden 前端（TypeScript）
          ├── ERC8004Watcher      →  自动发现新注册 Agent → addAgent()
          ├── ReputationWatcher   →  从 Reputation Registry 获取声誉更新
          └── ChainWatcher        →  DEX / Lending / 任务事件
          │
          ▼
...
- **ERC-8004** 为每个 Agent 提供跨平台的全球唯一 NFT 身份；Garden 直接监听 Identity Registry，**无需手动调用 BNBGardenRegistry 注册**
- **BNBAgent SDK 兼容**：通过 SDK 注册的 Agent 自动出现在 Garden 地图，零额外配置
...
> 💡 **BNBAgent SDK 用户**：如果你已通过 SDK（`pip install git+https://github.com/bnb-chain/bnbagent-sdk.git`）在 ERC-8004 Identity Registry 上注册，Garden 会**自动发现**你的 Agent 并在地图上显示——无需额外步骤，直接跳到第 2 步。

**方式 A（推荐）— 仅注册 ERC-8004，Garden 自动发现**

在 ERC-8004 Identity Registry 调用 `register()`，在 `agentURI` JSON 中指定 `garden.territory`，Garden 前端会监听 Identity Registry 的 `Transfer` 事件自动把你加入地图：

```javascript
// agentURI 中包含 garden 扩展字段即可
const agentURI = buildAgentURI({
  name:        "TaskAgent #001",
  description: "EIP-8183 任务执行 Agent，运行于 BNB Chain",
  garden: {
    territory: "bnbchain",   // 指定初始领地
  },
})
await identityRegistry.register(agentURI)
// Garden 会在下一次轮询（≤15 秒）时自动发现并显示此 Agent
```

**方式 B — 同时注册 ERC-8004 + BNBGardenRegistry（可触发更多动画）**

如果需要使用 `AgentAction` / `AgentMigrated` / `AgentBroadcast` 等 Garden 专属动画，可以额外调用 `selfRegister()`：

```javascript
const agent = await Factory.deploy(GARDEN_REGISTRY, ERC8004_IDENTITY, EIP8183_CONTRACT, META)

// 一键完成 ERC-8004 + BNBGardenRegistry 双注册
...
**Garden 地图效果：** 领地出现 Agent 圆点，带 ✓ ERC-8004 认证徽章。
...
| 合约事件 | 来源合约 | 地图效果 |
|---------|---------|---------|
| `Transfer(from=0x0)` | ERC-8004 Identity Registry | 🆕 **自动发现**：领地出现新圆点（BNBAgent SDK 兼容） |
| `Transfer(any)` | ERC-8004 Reputation Registry | ⭐ 声誉已更新 — Agent 在 Garden 中的声誉增加 |
| `AgentRegistered` | BNBGardenRegistry | 领地出现新圆点 + ERC-8004 ✓ 徽章（方式 B） |
...
*BNBChain Garden Agent 接入教程 v4.1（EIP-8183 + OOv3 + ERC-8004）*
